...
def trainModel(sess, accuracy, train_op, X, Y, trainX, trainY, testX, testY, merged_summary_op):
	# Launch session for training
	#with tf.Session() as sess:

		# op to write logs to Tensorboard
		#summary_writer = tf.train.SummaryWriter(logs_path, graph=tf.get_default_graph())
		summary_writer = tf.train.SummaryWriter(logs_path, sess.graph_def)
		train_summary  = tf.train.SummaryWriter(logs_path+'/train', sess.graph_def)
		test_summary   = tf.train.SummaryWriter(logs_path+'/test', sess.graph_def)
		...
		for indexIter in range(MAX_ITERS):
			for startIndex, endIndex in zip( range(0,len(trainX),BATCH_SIZE), range(BATCH_SIZE,len(trainX),BATCH_SIZE)):
				_, summary = sess.run([train_op, merged_summary_op], feed_dict={X: trainX[startIndex:endIndex], Y: trainY[startIndex:endIndex]})
				
				summaryIndex = summaryIndex+1
				# Write logs at every iteration (after each mini-batch log the training stats)
				summary_writer.add_summary(summary, summaryIndex)

			# Visualize accuracy only every 10 iterations
			if indexIter%2==0 :
				
				acc_train, summary_train = sess.run([accuracy, merged_summary_op], feed_dict={X:trainX, Y:trainY})
				train_summary.add_summary(summary_train, indexIter)
				acc_test, summary_test   = sess.run([accuracy, merged_summary_op], feed_dict={X:testX,  Y:testY})
				test_summary.add_summary(summary_test, indexIter)
...
def main():
	...
	# ===========================================================
	# TENSORBOARD - variables to monitor
	# ============================================================
	# Create a summary to monitor loss tensor
	tf.scalar_summary("loss", loss)
	# Create a summary to monitor accuracy tensor
	tf.scalar_summary("accuracy", accuracy)	
	## Create a summary to monitor class prediction accuracy(top 1)
	#tf.scalar_summary("prediction", agreement)	
	...
	with tf.Session() as sess:
		...
		# train model
		trainModel(sess, accuracy, train_op, X, Y, trainX, trainY, testX, testY, merged_summary_op)
